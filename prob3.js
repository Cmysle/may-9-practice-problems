// Phew, you've beaten back the zombies. But it's still the apocalypse, and there's still much more to do. Next up, we need to gather enough food and supplies to make it through goodness knows how long.
// It may be time to repurpose our JARVIS bot to gather some food and supplies for our base camp.

// Write a function called gatherSupplies that takes in one parameter called suppliesToLookFor, which you can expect will always be given an array as an argument value. That array will contain any number of strings,
// where each string represents a specific food or type of supply to look for. Your function should use some sort of loop to iterate through this array for each individual element.
...
function gatherSupplies(suppliesToLookFor) {
  for (i = 0; i < suppliesToLookFor.length; i++) {
    console.log("JARVIS has found some " + suppliesToLookFor[i] + "!");
  }
  console.log(suppliesToLookFor.length)
}

myArray = ["socks", "food", "water", "guns", "bandages"]

gatherSupplies(myArray);
